...
export default function mf(api: IApi) {
  ...
  api.modifyWebpackConfig(async (config, { webpack }) => {
    ...
    const remoteVersion = api.config.mf.version;
    const mfConfig = {
      name,
      remotes,
      filename: useHash
        ? 'remote.[contenthash:8].js'
        : remoteVersion
        ? `remote-${remoteVersion}.js`
        : 'remote.js',
      exposes,
      shared,
  ...
  api.onGenerateFiles(() => {
    api.writeTmpFile({
      ...
      __webpack_public_path__ = (document.currentScript.src.match(/\?.+=/); match ? document.currentScript.src.substring(match.index) : document.currentScript.src) + '/../';`,
